...
async function check() {
  // Try querying using exec_sql RPC
  const { data, error } = await client.rpc('exec_sql', { query: 'SELECT * FROM cron.job' });
  console.log('Cron Jobs Error:', error);
  console.log('Cron Jobs Data:', JSON.stringify(data, null, 2));
}
